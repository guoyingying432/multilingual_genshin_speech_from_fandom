...
Audio_PATH = os.path.join(FILE_PATH,"enAudio")
...
async def main():
    ...
    #print(raw_data)
    for i in raw_data:
        char_url = "https://genshin-impact.fandom.com" + i.find("a")["href"] + "/Voice-Overs"
        ...
        for g in audio_index:
            ...
            for k in voice_data[g].tbody.find_all("tr")[1:]:
                ...
                if len(k.find_all("td")) == 0:
                    continue
                if len(k.td.find_all("span",lang='en'))==0:
                    continue
                #print(k)
                #print(k.td.find_all("span",lang='en'))
                tmp=k.td.find_all("span",lang='en')
                #print(tmp[0])
                # Parse the HTML text
